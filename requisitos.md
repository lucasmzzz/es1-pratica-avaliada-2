...
| RF02 | O sistema deve permitir o cadastro de leitores com nome, CPF, e-mail e telefone. | Alta |
| RF03 | O sistema deve registrar o empréstimo de livros vinculando o ISBN do livro ao CPF do leitor. | Alta |
| RF04 | O sistema deve calcular automaticamente a data prevista de devolução no momento do empréstimo. | Alta |
| RF05 | O sistema deve permitir que um leitor reserve um livro caso todos os exemplares estejam emprestados. | Média |
| RF06 | O sistema deve registrar a devolução de livros emprestados. | Alta |
| RF07 | O sistema deve notificar por e-mail o primeiro leitor da fila de reservas quando o livro for devolvido. | Média |
| RF08 | O sistema deve permitir a renovação de empréstimos ativos. | Baixa |
| RF09 | O sistema deve impedir a renovação de um empréstimo caso exista uma reserva ativa para o livro. | Alta |
| RF10 | O sistema deve calcular e aplicar multas financeiras para devoluções realizadas após o prazo previsto. | Alta |
...
| ID | Descrição |
|----|-----------|
| RN01 | O prazo padrão para empréstimos e devoluções é fixado em exatos 14 dias. |
| RN02 | A multa por atraso na devolução é calculada no valor fixo de R$ 2,00 por dia de atraso. |
| RN03 | Uma reserva só pode ser efetivada se a quantidade de exemplares disponíveis do livro for igual a zero. |
| RN04 | A renovação de um empréstimo é estritamente proibida caso exista qualquer reserva pendente para a obra. |
| RN05 | A fila de reservas deve respeitar a ordem cronológica (FIFO), notificando sempre o leitor mais antigo da fila. |
